...
export default function HeroModel() {
  ...
  useEffect(() => {
    ...
    import('three').then((THREE) => {
      ...
      const onTouch=e=>{ if(!e.touches.length)return; pointer.x=(e.touches[0].clientX/window.innerWidth)*2-1; pointer.y=-(e.touches[0].clientY/window.innerHeight)*2+1; };
      window.addEventListener('mousemove',onMouse,{passive:true,capture:false});
      window.addEventListener('touchmove',onTouch,{passive:true});

      const onResize=()=>{
        const w=el.clientWidth||500, h=el.clientHeight||500;
        camera.aspect=w/h;
        // Widen FOV for tall/square containers so nothing gets clipped
        camera.fov = w/h < 1 ? 58 : 42;
        camera.updateProjectionMatrix();
        renderer.setSize(w,h);
      };
      window.addEventListener('resize',onResize,{passive:true});
      onResize(); // apply correct FOV immediately
      ...
      const loop=()=>{
        ...
        coreWire.rotation.y+=0.008; coreWire.rotation.z+=0.005;
